...
export function clearTempTags(input) {
  const list = ['twig'];
  return input.replace(new RegExp(`<\\/?${list.join('|')}>`, 'gmi'), '');
}
...
function getMarkdownitService(options) {
  ...
  md.renderer.rules.html_block = (tokens, index) => {
    ...
    const supSubScript = new RegExp('(<su[b|p]>)', 'i');

    if (output.match(htmlEmbed) || output.match(oEmbed) || output.match(supSubScript)) {
      return output;
    }
...
function getTurndownService(options) {
  ...
  td.addRule('html_snippet', {
    ...
    replacement: (content, node) => `\n\n${formatHTML(clearTempTags(node.outerHTML))}\n\n`,
  });
