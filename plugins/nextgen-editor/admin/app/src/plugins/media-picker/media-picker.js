import { insertImage } from '../image/utils/utils';
...
const selectHandler = (event) => {
  ...
  const val = $(event.target).parents('.js__media-element').data('file-url');

  modal.dataField.value = val.replace(/ /g, '%20');
...
export default function showMediaPicker(callback) {
  ...
  const modalElement = body.find('[data-remodal-unique-identifier]');
  modal = jQuery.remodal.lookup[modalElement.data('remodal')];
  ...
  modal.open();
  body.off('click', '[data-remodal-mediapicker] .media-container.in-modal .admin-media-details a');
  body.on('click', '[data-remodal-mediapicker] .media-container.in-modal .admin-media-details a', selectHandler);
  modal.dataField = document.querySelector('#ck-media-picker input[name="data[ck-media-picker]"]');

  const { page } = document.querySelector('#ck-media-picker').dataset;

  const item = document.querySelector(`[data-remodal-mediapicker] [data-nav-id="${page}"] [data-page]`);
  item.click();
  document.querySelector('#admin-media').style.visibility = 'visible';
}
