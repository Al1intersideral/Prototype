import { insertMedia } from '../utils/utils';
  ...
  init() {
    ...
    if (!INITIALIZED) {
      ...
      mediaContainer.delegate('[data-dz-insert]', 'click', async (event) => {
        ...
        await insertMedia(this.editor, filename);
      });
      ...
      dropzone.on('thumbnail', (file) => {
        if (file.nextgenEditor) {
          file.nextgenEditor.model.change((writer) => {
            file.modelTempImage = writer.createElement('imageBlock', { src: file.dataURL });
      ...
      dropzone.on('success', async (file, resp) => {
        if (file.nextgenEditor) {
          ...
          await insertMedia(file.nextgenEditor, resp.filename);
    ...
    this.editor.editing.view.document.on('drop', async (event, data) => {
      ...
      if (filename) {
        ...
        await insertMedia(this.editor, filename, position);
      }
    });
  },
});
